...
// Put into auto mode
  tmp[0] = 0x00;
  tmp[1] = 0x00;
  _i2c_bus->writeBytes(LC709204F_ADDRESS, LC709204F_CURRENT_DIR,  2, &tmp[0]);        // set auto mode

// Set empty cell voltage to 3.61 V
  tmp[0] = 0x1A;
  tmp[1] = 0x0E;
  _i2c_bus->writeBytes(LC709204F_ADDRESS, LC709204F_EMPTY_CELL_VOLTAGE,  2, &tmp[0]); // set empty cell voltage
...
uint16_t LC709204F::timetoEmpty()
{
  ...
  uint16_t Etime = (uint16_t) ((uint16_t)tmp[1] << 8) | tmp[0];
  return Etime;
}
